...
public class Menu implements Model {
	...
	public void addItem(MenuItem pItem) {
		if (aMenuItems.stream().map(m -> m.getName())
	...
	public void removeItem(String pName) {
		aMenuItems.removeIf(m -> m.getName().toLowerCase().equals(pName));
	}
	...
	/**
	 * A filter method that returns a list of MenuItem from the Menu that has a
	 * price lower than pPrice
	 * 
	 * @pre pPrice >= 0
	 */
	public List<MenuItem> filter(double pPrice) {
		...
		List<MenuItem> res = aMenuItems.stream().filter(mi -> mi.getPrice() < pPrice).collect(Collectors.toList());

		return res;
	}

	/**
	 * A filter method that returns a list of MenuItem from the Menu that has the
	 * FoodCategory pCategory
	 * 
	 * @pre pCategory != null
	 */
	public List<MenuItem> filter(FoodCategory pCategory) {
		...
		List<MenuItem> res = aMenuItems.stream().filter(mi -> mi.getFoodCategory().equals(pCategory))
	...
	/**
	 * A filter method that returns a list of MenuItem from the Menu that has the
	 * combination of Dietary Category pDietCategory
	 * 
	 * @pre pDietCategory != null
	 */
	public List<MenuItem> filter(Set<DietaryCategory> pDietCategory) {

		assert pDietCategory != null;

		List<MenuItem> res = aMenuItems.stream().filter(mi -> mi.getDietaryCategories().equals(pDietCategory))
				.collect(Collectors.toList());
